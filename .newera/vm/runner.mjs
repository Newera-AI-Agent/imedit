// .newera/vm/runner.mjs — NewEra headless coding agent v3.6 (runs inside GitHub Actions).
...
const SUB_SYSTEM_PROMPT = [
  ...
  "",
  "TOOLS: shell{command, timeout_minutes?}, list_files{path?}, read_file{path,start?,end?}, write_file{path,content}, append_file{path,content}, edit_file{path,find,replace,all?}, delete_file{path}, generate_image{prompt,path,size?}. For files over ~120 lines: write_file the first chunk, then append_file the rest — one giant JSON string breaks the protocol.",
  "",
  "RULES:",
...
async function runSubagent(spec, parentStep) {
  ...
  for (step = 1; step <= SUB_MAX_STEPS; step++) {
    ...
    try {
      ...
      else result = { ok: false, output: "unknown tool: " + toolName + " — available: shell, list_files, list_skills, read_skill, read_file, write_file, append_file, edit_file, delete_file, generate_image" };
    } catch (err) {
...
const SYSTEM_PROMPT = [
  ...
  "- finish{summary} — task complete. Requires: dependencies installed, build/test commands actually run and passing (or the exact blocker documented). Aliases final_output / final_report / final_result / complete / done / stop are accepted, but plain finish is the contract.",
  "",
  "## RULES",
  ...
  "Polish the UI like a designer, not a template: coherent spacing and typography, dark pro-tool aesthetics when the brief says editor/tool, empty/loading/error states everywhere, and REAL visual assets via generate_image (og-image, empty-state art, icons) instead of bare divs. A working app with an amateur look is not done.",
].join("\n");
...
async function main() {
  ...
  for (step = 1; step <= MAX_STEPS; step++) {
    ...
    try {
      ...
      else if (toolName === "append_file") { result = toolAppendFile(args); wroteSinceCommit = true; }
      ...
      else if (toolName === "generate_image") { result = await toolGenerateImage(args); wroteSinceCommit = true; }
